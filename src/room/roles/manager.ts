...
@creepRole("manage")
export class RoleManager extends CreepRoleBase {
    @memorize
    target: string;
    lastScanned = 0;
    tasks: ((room: RoomInfo, storage: StructureStorage, capacity: number) =>
        { from: AnyStoreStructure, to: AnyStoreStructure, type: ResourceConstant, amount?: number })[] = [
            ...
            (room, storage) => {
                if (Game.time - this.lastScanned > 20) {
    ...
    run(creep: Creep, room: RoomInfo) {
        ...
        if (this.target) {
            let target = Game.getObjectById(this.target) as AnyStoreStructure;
    ...
    static defaultBody: BodyPartDescription = [[CARRY, 16]];

    static spawnInfo(room: RoomInfo) {
        if (room.structRcl >= 6) {
            return {
                "center": this.defaultBody
            }
        }
    }
}
